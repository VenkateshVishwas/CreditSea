...
import { UserDashboardResponse } from "./login/types";
...
function UserDashboard() {
    ...
    useEffect(() => {
        const handleData = async () => {
            try {
                const response = await axios.get<UserDashboardResponse>(
                    "https://creditsea-backend-ymi7.onrender.com/api/dashboard/user",
                    { withCredentials: true }
                );

                const resData = response.data;

                if (resData.applications) {
                    const result: LoanDetails[] = resData.applications.map((i) => ({
                        "Loan Officer": i.reason, // or whatever makes sense here
                        "Amount": `₹${i.amount}`, // optional formatting
                        "Date Applied": new Date(i.createdAt).toLocaleDateString(),
